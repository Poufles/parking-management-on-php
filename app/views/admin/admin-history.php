...
<form method="GET" action="<?= APP_URL ?>admin/history"
    ...
    <div style="display:flex; flex-wrap:wrap; gap:12px; align-items:flex-end;">
        ...
        <div style="display:flex; flex-direction:column; gap:4px;">
            <label style="font-size:12px; color:#666; font-weight:600;">FROM</label>
            <input type="date" name="date_from" class="form-control"
                   value="<?= htmlspecialchars($dateFrom) ?>">
        </div>
        <div style="display:flex; flex-direction:column; gap:4px;">
            <label style="font-size:12px; color:#666; font-weight:600;">TO</label>
        ...
        <div style="display:flex; flex-direction:column; gap:4px;">
            <label style="font-size:12px; color:#666; font-weight:600;">ACCOUNT TYPE</label>
            <select name="filter_acct" class="form-select" style="width:130px;">
                <option value="">All</option>
                <option value="client" <?= $filterAcct === 'client' ? 'selected' : '' ?>>Client</option>
                <option value="admin"  <?= $filterAcct === 'admin'  ? 'selected' : '' ?>>Admin</option>
            </select>
        </div>

        <div style="display:flex; gap:8px; align-items:flex-end;">
            <button type="submit" class="btn btn-secondary">Apply</button>
            <a href="<?= APP_URL ?>admin/history" class="btn btn-outline-secondary">Clear</a>
        </div>
            ...
            <?php else: ?>
                <?php foreach ($rows as $index => $data): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="badge <?= $data['account_type'] === 'admin' ? 'bg-danger' : 'bg-success' ?>">
